 ...
 * - Adversarial negative/tamper checks (Wycheproof-style methodology; no imported corpus)
...
try {
  ...
  console.log('\n▶ [TIER 6] Adversarial Negative & Tamper Tests:');
  ...
  pass('x402 dual conjunction holds when Ed25519 and ML-DSA are both valid');
  ...
  pass('x402 dual conjunction fails-closed when the classical signature is compromised');

  const failClosedOk = classicalValid && badSigVer;
  assert.strictEqual(failClosedOk, false);
  pass('x402 dual conjunction fails-closed when the PQC component is compromised');
