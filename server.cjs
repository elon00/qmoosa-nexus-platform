...
var OFFICIAL_NEXUS_RECIPIENT = "8qhW8ctXX77UNLTY9kx3XoAoH8kstQXPbCghUwqu34es";
var USED_NEXUS_SIGNATURES = /* @__PURE__ */ new Set();
    ...
    endpoints: [
      {
        ...
        pricing: { amountSol: 1e-3, lamports: 1e6, currency: "SOL", alternativeUsdc: "0.01" }
      },
      {
...
app.post("/api/v1/x402/agent/plan", async (req, res) => {
  ...
  return res.json({
    ...
    x402Receipt: { signature, recipient: OFFICIAL_NEXUS_RECIPIENT, amountSol: 1e-3 },
    plan: {
...
app.post("/api/v1/x402/policy/audit", async (req, res) => {
  const authHeader = req.headers["authorization"] || "";
  const sigHeader = req.headers["x-payment-signature"] || "";
  let signature = "";
  if (typeof authHeader === "string" && authHeader.toLowerCase().startsWith("x402 ")) {
